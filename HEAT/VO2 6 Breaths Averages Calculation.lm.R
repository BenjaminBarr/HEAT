...
df <- read_excel("./HEAT/Jaden.VO2.peak.xlsx")
names(df)
...
names(df2) <- c("vo2", "hr", "watts")

df3 <- df2 %>%
  mutate(smooth_y = predict(gam(hr ~ s(vo2, bs = "cs"), data = df2))) %>%
  mutate(lm_y = predict(lm(hr ~ vo2), data = df2))
...
vo2.range$vo2 <- vo2.range$per*35.40167

# Fit a LOESS model
#loess_model <- loess(hr ~ vo2, data = df)
loess_model <- loess(hr ~ vo2, data = df3)
lm_model <- lm(hr ~ vo2, data = df3)
...
ggplot(df2, aes(x = vo2, y = hr)) +
  geom_point(data = df2, aes(color = watts)) +
